...
int handle_lupineDedupTransfer(conn_t *conn) {
  ...
  try {
    staging->data.resize(staging->total_bytes);
  ...
  for (uint32_t index = 0; index < begin.chunk_count; ++index) {
    ...
    if (hit) {
      std::memcpy(staging->data.data() + hash.offset, cached.data(),
                  cached.size());
...
int handle_lupineDedupBulkChunk(conn_t *conn) {
  ...
  if (valid) {
    ...
    valid = !staging->failed && header.total_bytes == staging->total_bytes &&
            range_for_sequence(*staging, header.sequence, header.offset,
                               header.bytes) &&
  ...
  if (!valid || header.bytes > SIZE_MAX) {
    if (header.bytes > LUPINE_DEDUP_CHUNK_BYTES || header.bytes > SIZE_MAX ||
        rpc_drain(conn, static_cast<size_t>(header.bytes)) < 0 ||
        rpc_read_end(conn) < 0) {
      return -1;
    }
  ...
  auto *destination = staging->data.data() + header.offset;
  if (rpc_read(conn, destination, static_cast<size_t>(header.bytes)) < 0 ||
      rpc_read_end(conn) < 0) {
    fail_staging(header.copy_id);
    return -1;
  }
  lupine_dedup_key actual =
      lupine_dedup_hash(destination, static_cast<size_t>(header.bytes));
  ...
  lupine_dedup_cache_admit(actual, destination);
  staging_progress().notify_all();
  return 0;
}
...
int handle_lupineDedupCommit(conn_t *conn) {
  ...
  if (staging != nullptr) {
    ...
    if (!staging->failed) {
      result = cuMemcpyHtoD_v2(static_cast<CUdeviceptr>(staging->destination),
                               staging->data.data(), staging->total_bytes);
    } else {
