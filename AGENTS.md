...
- Valid ComfyUI GPU-over-IP testing requires the client container to omit `--gpus all`, set `NVIDIA_VISIBLE_DEVICES=void`, and expose GPUs only through `LUPINE_SERVER`; otherwise workloads execute locally and are invalid for bandwidth testing.
